...
try {
     ...
     if (!blog) {
      ...
      return Response.json({ message: "Blog fetched successfully", success: true, catagoryblog: blog }, { status: 200 });
} catch (error) {
    
    return Response.json({ message: "Blog fetching failed", success: false }, { status: 500 });
}
